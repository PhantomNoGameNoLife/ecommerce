...
const ProductCard =  ({product}:{product:Product}) => {
  return (
    <div className="w-full max-w-sm bg-[var(--card)] border border-[var(--border)] rounded-lg shadow-sm">
        ...
        <div className="p-5">
            <h4 className="text-md font-bold tracking-tight text-[var(--chart-1)] line-clamp-1 mb-1">{product.category.name}</h4>
            <Link href={`/productDetails/${product.id}`}>
                <h5 className="text-xl font-semibold tracking-tight text-[var(--primary)] line-clamp-1">{product.title}</h5>
            </Link>
            ...
            <div className="flex items-center justify-between">
                <span className="text-2xl font-bold text-[var(--primary)]">{product.price}<sub>EGP</sub></span>

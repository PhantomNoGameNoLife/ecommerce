...
const OrderItem = ({ item }: { item: CartItem }) => {
    return (
        <div className="flex flex-col lg:flex-row items-center py-6 border-b border-border gap-6 w-full">
            <Link href={`/productDetails/${item.product.id}`}>
                <div className="img-box max-lg:w-full relative">
                    <Image fill src={item.product.imageCover} alt={item.product.title}
                        className="aspect-square w-full lg:max-w-[140px] rounded-xl object-cover !relative !h-[200px] lg:!h-auto" />
                </div>
            </Link>
            <div className="flex flex-row items-center w-full ">
                <div className="grid grid-cols-1 lg:grid-cols-2 w-full">
                    <div className="flex items-center">
                        <div>
                            <h2 className="font-semibold text-xl leading-8 text-foreground mb-3">
                                {item.product.title}
                            </h2>
                            ...
                            <div className="flex items-center ">
                                <p className="font-medium text-base leading-7 text-foreground pr-4 mr-4 border-r border-border">
                                    Ratings:
                                    <span className="text-muted-foreground">{item.product.ratingsAverage}</span>
                                    <Star className='text-amber-400 fill-amber-400 ms-2' />
                                </p>
                                <p className="font-medium text-base leading-7 text-foreground pr-4 mr-4 border-r border-border">
